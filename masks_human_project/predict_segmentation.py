...
def verify_vehicle(image_path: str) -> bool:
    ...
    try:
        ...
        # Require minimum 40% confidence for vehicle
        if best_conf < 0.40:
...
def _run_yolo_seg(image_path: str, model_path: str, conf: float = 0.75):
    ...
    try:
        ...
        results = model(image_path, verbose=False, conf=conf,
                        iou=0.45, imgsz=640, max_det=20)[0]
...
def _local_analyze(image_path: str, sensitivity: float = 65.0) -> dict:
    ...
    if len(polygons) > 0:
        ...
        elif cv2_score < 8.0 and max_yolo_conf < 0.85:
            # Low CV2 + low-ish YOLO conf = likely false positive
...
def predict(image_path: str,
    ...
    # ── Step 0: Vehicle Verification ──
    if not verify_vehicle(image_path):
        return {
    ...
    # ── Step 2: Reuse YOLO polygons from analysis (no double inference) ──
